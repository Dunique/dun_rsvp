...
class Dun_rsvp_helper
{
	...
	*/
     ...
     * @access private
    */
    public static function set_ee_cache($name = '', $value = '')
    {
    	if ( isset(ee()->session->cache[FALLBACK_FIELD_MAP][$name]) == FALSE )
		{
			ee()->session->cache[FALLBACK_FIELD_MAP][$name] = $value;
		}
		return ee()->session->cache[FALLBACK_FIELD_MAP][$name];
    ...
    */
	public static function mcp_meta_parser($type='', $file)
	{
		...
		if ($type == 'css')
		{
			if ( isset(ee()->session->cache[DUN_RSVP_MAP]['CSS'][$file]) == FALSE )
			{
				ee()->cp->add_to_head('<link rel="stylesheet" href="' . ee()->dun_rsvp_settings->get_setting('theme_url') . 'css/' . $file . '" type="text/css" media="print, projection, screen" />');
				ee()->session->cache[DUN_RSVP_MAP]['CSS'][$file] = TRUE;
			}
		}
		...
		if ($type == 'js')
		{
			if ( isset(ee()->session->cache[DUN_RSVP_MAP]['JS'][$file]) == FALSE )
			{
				ee()->cp->add_to_foot('<script src="' . ee()->dun_rsvp_settings->get_setting('theme_url') . 'javascript/' . $file . '" type="text/javascript"></script>');
				ee()->session->cache[DUN_RSVP_MAP]['JS'][$file] = TRUE;
			}
		}
    ...
    {
        if (
            ee()->dun_rsvp_settings->item('report_stats') != 0 && 
            function_exists('curl_init') &&
            ee()->dun_rsvp_settings->item('report_date') <  ee()->localize->now)
        {
            $data = http_build_query(array(
                // anonymous reference generated using one-way hash
                'hash' => isset($overide['hash']) ? $overide['hash'] : md5(ee()->dun_rsvp_settings->item('license_key').ee()->dun_rsvp_settings->item('site_url')),
                'license' => isset($overide['license']) ? $overide['license'] : ee()->dun_rsvp_settings->item('license_key'),
                'product' => isset($overide['product']) ? $overide['product'] : DUN_RSVP_NAME,
                'version' => isset($overide['version']) ? $overide['version'] : DUN_RSVP_VERSION,
                'ee' => APP_VER,
                'php' => PHP_VERSION,
                'time' => ee()->localize->now,
            ));
            ee()->load->library('curl');
            ee()->curl->simple_post(DUN_RSVP_STATS_URL, $data);
            //ee()->curl->debug();

            // report again in 7 days
            ee()->dun_rsvp_settings->save_setting('report_date', ee()->localize->now + 7*24*60*60);
        }
    }
	...
	public static function license_check()
	{
		...
		{
			if (preg_match($pattern, ee()->dun_rsvp_settings->item('license_key')))
			{
				$is_valid = TRUE;
				break;
			}
