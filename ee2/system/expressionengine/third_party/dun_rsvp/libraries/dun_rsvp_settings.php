...
class Dun_rsvp_settings {
	...
	public function __construct()
	{
		...
		$this->default_settings = array(
			'module_dir'   => PATH_THIRD.DUN_RSVP_MAP.'/',
			'theme_dir'   => PATH_THEMES.'third_party/'.DUN_RSVP_MAP.'/',
		...
		{
			//set the theme url
			$theme_url = ee()->config->slash_item('theme_folder_url') != '' ? ee()->config->slash_item('theme_folder_url').'third_party/'.DUN_RSVP_MAP.'/' : ee()->config->item('theme_folder_url') .'third_party/'.DUN_RSVP_MAP.'/'; 
		...
		{
			//set the Theme dir
			$this->default_settings['theme_url'] = URL_THIRD_THEMES.DUN_RSVP_MAP.'/';
		}
		...
		if(REQ == 'CP' && isset(ee()->db))
		{
			...
			$this->default_settings['base_url'] = BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module='.DUN_RSVP_MAP;		
			$this->default_settings['base_url_js'] = '&C=addons_modules&M=show_module_cp&module='.DUN_RSVP_MAP;
		}
	...
	{	
		...
		//insert into db
		ee()->db->insert_batch(DUN_RSVP_MAP.'_settings', $data);
	...
	{
		if (ee()->db->table_exists(DUN_RSVP_MAP.'_settings'))
        {
        	//check if there is any result
        	$check = ee()->db->from(DUN_RSVP_MAP.'_settings')->get();
        	if($check->num_rows() > 0)
        	{
				//get the site IDS
				...
				foreach($sites as $site_id)
				{
					foreach($this->default_post as $key=>$val)
					{
						...
						$q = ee()->db->get(DUN_RSVP_MAP.'_settings');
						...
						if($q->num_rows() == 0)
						{
							...
							//insert into db
							ee()->db->insert(DUN_RSVP_MAP.'_settings', $data);
						}
					}
				}
			}
	...
	public function load_settings($all_sites = TRUE)
	{
		if (ee()->db->table_exists(DUN_RSVP_MAP.'_settings'))
        {
			...
			$get_setting = ee()->db->get_where(DUN_RSVP_MAP.'_settings', array(
				'site_id' => $this->default_settings['site_id']
			));
			...
			{
				//is serialized?
                if(call_user_func(array(DUN_RSVP_MAP.'_helper','is_serialized'), $row->value))
                {
    ...
    public function get_db_settings($site_id = '', $setting_name = '')
    {
		...
		ee()->db->from(DUN_RSVP_MAP.'_settings');
		$q = ee()->db->get();
   ...
   // ----------------------------------------------------------------
	...
	public function save_post_settings()
	{
		...
		//redirect
		ee()->functions->redirect(BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module='.DUN_RSVP_MAP.AMP.'method=settings');			
	}
	...
	public function save_setting($key = '', $val = '')
	{
		if($key != '')
		{
			...
			//update the record
			ee()->db->update(DUN_RSVP_MAP.'_settings', array(
				'value' => $val
			));		
		}
	}
